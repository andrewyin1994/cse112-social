...
const docRef = firestore.doc("team/schedule")
...
class Schedule {
	...
	addSchedule(name, number, position,archived,load) {
		let scheduleEl = new scheduleElement(name, number, position, archived);
		this.schedule.push(scheduleEl);
		window.localStorage["schedule"] = JSON.stringify(TeamSnip.currentSchedule.schedule);
		console.log(load);
		if(load) {
			docRef.set({team: window.localStorage["schedule"]});
		}
	}
	...
	renderAddForm(playerId) {
		...
		document.querySelector("#addPlayerBtn").addEventListener("click", function () {
			...
			if (playerId) {
				...
				TeamSnip.currentSchedule.addSchedule(name,number,position,false,1);
			}
	...
	if(!navigator.onLine) {   
		if(window.localStorage["loadedSched"]) {
			...
			for (let i = 0; i < out.length; i++) {
				TeamSnip.currentSchedule.addSchedule(out[i].name,out[i].number,out[i].position,s[i].archived,0);
			}
			TeamSnip.currentSchedule.render();
		}
	}
		...
		docRef.get().then(function(doc) {
			if (doc && doc.exists) {
				const myData = doc.data().team;
				if(myData) {
					var s = JSON.parse(myData);
					window.localStorage["schedule"] = s;
					for (let i = 0; i < s.length; i++) {
						TeamSnip.currentSchedule.addSchedule(s[i].name,s[i].number,s[i].position,s[i].archived,0);
					}
				}
			}
			else {
